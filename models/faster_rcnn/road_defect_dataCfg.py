...
DETECTRON2_DATASETS = '/mnt/c/WORKING_FOLDER/PROJECTS/MTECH/GITHUB/roaddefectdetector/SystemCode/roaddefectdetector/src/data'
ROADDEFECT_DATASET  = DETECTRON2_DATASETS
DATASET_BASE_PATH = ROADDEFECT_DATASET

_PREDEFINED_SPLITS_GRC_MD = {}

# Original training data set with image annotation
_PREDEFINED_SPLITS_GRC_MD["roaddefect_source"] = {
    "roaddefect_train": ( "train/Czech", 
                          "train/India", 
                          "train/Japan")
}

# Splitted  training data set to train/validation/test with image annotation
_PREDEFINED_SPLITS_GRC_MD["roaddefect"] ={
    "roaddefect_train": ( 
                       "ltrain/Czech", 
                       "ltrain/India", 
                       "ltrain/Japan"
...
_PREDEFINED_SPLITS_GRC_MD["roaddefect_new"] = {
    "roaddefect_new_test1": ( "test1/Czech", 
                              "test1/India", 
                              "test1/Japan"),
    "roaddefect_new_test1": ( "test2/Czech", 
                              "test2/India", 
                              "test2/Japan")
}

RDD_DEFECT_CATEGORIES_ALL=[
...
def prepare_target_directories(basepath, dataset_name, splits_per_dataset):
    for dataset_name, splits_per_dataset in _PREDEFINED_SPLITS_GRC_MD["roaddefect"].items():
            ...
            for run_id, img_file_idx in enumerate(img_file_idxs):
                ...
                if os.path.isfile(os.path.join(ann_fullfile)):
                    if run_id < train_limit: 
                        target_basepath = _PREDEFINED_SPLITS_GRC_MD["roaddefect"]["roaddefect_train"][idx] 
                    elif run_id < val_limit:
                        target_basepath = _PREDEFINED_SPLITS_GRC_MD["roaddefect"]["roaddefect_val"][idx]                     
                    else:
                        target_basepath = _PREDEFINED_SPLITS_GRC_MD["roaddefect"]["roaddefect_test"][idx]
...
def check_dataset_info(basepath, dataset_name, splits_per_dataset):
    ...
    for idx, regions_data in enumerate(splits_per_dataset): # Region specific
        ...
        if os.path.isdir(img_path):
            img_file_list = [filename for filename in os.listdir(img_path) if filename.endswith('.jpg')]
            img_file_idxs = list(range(len(img_file_list)))
            print(idx,"\tTotal:", len(img_file_list), ". Images from path = ", img_path)                    

# -------------- Detectron Loaders ------------------ #

def get_rdd_coco_instances_meta(categories):
    thing_ids = [k["id"] for k in categories]
    thing_names = [k["name"] for k in categories]
...
def load_images_ann_dicts(basepath, splits_per_dataset, categories):
    dataset_dicts = []
    thing_ids   = [k["id"] for k in categories]
    thing_names = [k["name"] for k in categories]
    image_id_count = 0
...
"""
Prepare data split (80:15:5) 
"""
if __name__ == "__main__":
    print("\n-----------", DATASET_BASE_PATH, "-----------\n")
    prepare_target_directories(DATASET_BASE_PATH, "roaddefect_source", _PREDEFINED_SPLITS_GRC_MD["roaddefect_source"]["roaddefect_train"])
    # Setup dataset for test/train/val split
    for dataset_name, splits_per_dataset in _PREDEFINED_SPLITS_GRC_MD["roaddefect_source"].items():
